...
import { getProviderById, createImageRecordOnce, completeImageRecord, failImageRecord, getImageByIdempotencyKey, getImagesByBatchId, ProviderConfig, ImageRecord } from "@/lib/db";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const clientKey = normalizeIdempotencyKey(req.headers.get("Idempotency-Key") || body.idempotency_key || body.idempotencyKey);
    if (!clientKey) {
    ...
    return handleBatch(user, provider, { prompt: prompt.trim(), aspect_ratio, resolution, quality, clientKey, price, count });
  } catch (e: unknown) {
...
function singleExistingResponse(img: ImageRecord) {
  if (img.status === "completed" && img.filename) {
    return NextResponse.json({ images: [imagePayload(img)], charged_vnd: img.cost_vnd, count: 1 });
  }
  if (img.status === "processing") return processingResponse();
...
async function handleSingle(
  ...
  if (existing) return singleExistingResponse(existing);
  ...
  if (!created) return singleExistingResponse(image);

  let charged = false;
  try {
    if (user.role !== "admin") {
      await debitForImage(user.id, image.id, opts.price);
      charged = true;
    }

    const results = await generateImage(provider, { prompt: opts.prompt, width, height, quality: opts.quality as "standard" | "high", aspectRatio: opts.aspect_ratio, resolution: opts.resolution });
    ...
    return NextResponse.json({ images: [imagePayload(record)], charged_vnd: charged ? opts.price : 0, count: 1 });
  } catch (e: unknown) {
    const message = e instanceof Error ? e.message : "Lỗi tạo ảnh";
    await failImageRecord(image.id, message).catch(() => undefined);
    if (charged) await refundForImage(user.id, image.id, opts.price, message).catch(() => undefined);
...
async function handleBatch(
  ...
  try {
    ...
    const results = await generateImage(provider, {
      prompt: opts.prompt, width, height, quality: opts.quality as "standard" | "high",
      aspectRatio: opts.aspect_ratio, resolution: opts.resolution, count: opts.count,
    });
    ...
    for (let i = 0; i < results.length && i < imageRecords.length; i++) {
      try {
        ...
        const record = await completeImageRecord(imageRecords[i].id, { filename: file.filename, mimeType: file.mimeType, model: results[i].model });
    ...
    if (charged && failedCount > 0 && completedImages.length > 0) {
      await refundForBatch(user.id, batchId, failedCount * opts.price, `${failedCount}/${opts.count} ảnh thất bại`).catch(() => undefined);
    }
