...
export default function AdminPage() {
  ...
  useEffect(() => { fetchUsers(); fetchNotice(); }, [fetchUsers, fetchNotice]);
    ...
    <AppShell>
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => { fetchUsers(); fetchNotice(); }} className="px-3 py-1.5 rounded-lg bg-zinc-800 text-sm text-zinc-300 hover:bg-zinc-700 cursor-pointer">
            Làm mới
          </button>
        </div>
        {error && <div className="rounded-xl border border-red-900/60 bg-red-950/30 px-4 py-3 text-sm text-red-200">{error}</div>}

        <ImageStatsPanel scope="all" title="Thống kê ảnh · toàn site" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6">
            ...
            {loading ? (
              ...
              <div className="overflow-x-auto">
                <div className="px-4 py-2 text-xs text-zinc-600 border-b border-zinc-800">
                  {visibleUsers.length}/{users.length} user
                </div>
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-zinc-800">
                    {visibleUsers.map((user) => (
                      <tr key={user.id} onClick={() => fetchUser(user.id)} className="hover:bg-zinc-800/50 cursor-pointer">
                        <td className="px-4 py-3">
                          <p className="text-zinc-200">{userLabel(user)}</p>
                          <p className="text-xs text-zinc-600">{user.email || user.phone || user.id}</p>
                        </td>
                        <td className="px-4 py-3 text-zinc-400">{user.role} · {user.status}</td>
          ...
          <aside className="rounded-xl border border-zinc-800 bg-zinc-900 p-5 space-y-5">
            {selected ? (
              <>
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="font-medium text-zinc-100">{userLabel(selected)}</h2>
                    {selected.status === "blocked" && (
                      <span className="text-[10px] px-1.5 py-0.5 bg-red-500/20 text-red-400 rounded">đã khoá</span>
                    )}
                  </div>
                  <p className="text-xs text-zinc-600 mt-1">Tạo ngày {formatDate(selected.created_at)}</p>
                  <p className="text-sm text-zinc-400 mt-3">Số dư hiện tại: <span className="text-zinc-100">{formatVnd(selected.balance_vnd)}</span></p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    <button onClick={() => setConfirmAction("toggle_status")} disabled={statusSaving}
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="VD: Nạp thủ công chuyển khoản"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => handleAdjust("topup")} disabled={saving || Number(amount) <= 0}
                    ...
                    <button onClick={() => handleAdjust("adjust")} disabled={saving || Number(amount) === 0}
                ...
                <div>
                  ...
                  {ledger.length === 0 ? <p className="text-sm text-zinc-600">Chưa có giao dịch</p> : (
              ...
              <p className="text-sm text-zinc-500">Chọn một user để xem ledger và cộng tiền.</p>
            )}
          </aside>
        </div>
      </main>
