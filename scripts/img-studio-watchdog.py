...
def collect_fail_clusters() -> list[tuple[str, str, int, str]]:
    """Trả list (model, error_norm, count, sample_email)."""
    sql = f"""
...
def collect_stuck_images() -> list[tuple[str, str, str, str]]:
    sql = f"""
SELECT r.id, COALESCE(r.model,'(none)'), COALESCE(u.email,'(no-email)'),
...
def collect_stuck_videos() -> list[tuple[str, str, str, str]]:
    sql = f"""
SELECT r.id, COALESCE(r.model,'(none)'), COALESCE(u.email,'(no-email)'),
       to_char(r."createdAt" AT TIME ZONE 'Asia/Ho_Chi_Minh', 'YYYY-MM-DD HH24:MI:SS')
FROM "RequestLog" r
...
def collect_missing_cpa_models() -> list[str]:
    """So provider.enabled.model với /v1/models của CPA (nếu có key)."""
    sql = """
...
"""
    ...
    # lấy base/key từ dòng đầu (cùng CPA)
    first = lines[0].split("\t")
    if len(first) < 3:
        return []
    ...
    for line in lines:
        parts = line.split("\t")
        model = parts[0]
        # Gemini/Imagen image qua chat; vẫn cần model tồn tại trên CPA list
        if model not in available:
            missing.append(model)
    return sorted(set(missing))


def collect_flow_pool() -> str | None:
    """Optional: đếm account reauth qua bridge admin nếu có env."""
    # Không bắt buộc — skip nếu không cấu hình
    return None


def build_alerts() -> list[tuple[str, str]]:
    """Trả list (dedup_key, message)."""
    alerts: list[tuple[str, str]] = []
    now_vn = datetime.now(timezone.utc).astimezone().strftime("%Y-%m-%d %H:%M %Z")

    # 1) Cluster fail cùng pattern
    ...
    if rate:
        ...
        if total >= FAIL_RATE_MIN_TOTAL and failed / total >= FAIL_RATE_THRESHOLD:
            key = f"failrate:{WINDOW_MINUTES}:{failed}:{total}"
            # dedup thô theo bucket 15p — dùng fixed key
    ...
    if stuck_img:
        lines = [f"  - {m} | {e} | {t}" for _, m, e, t in stuck_img[:8]]
        key = "stuck:image"
        msg = (
    ...
    if stuck_vid:
        lines = [f"  - {m} | {e} | {t}" for _, m, e, t in stuck_vid[:8]]
        key = "stuck:video"
        msg = (
    ...
    # 4) Model provider DB không còn trên CPA
    missing = collect_missing_cpa_models()
    if missing:
        key = "cpa:missing:" + ",".join(missing)
        # dedup theo tập model
        key = "cpa:missing"
        msg = (
...
def main() -> int:
    ...
    # merge process env (cron có thể export)
    ...
    if env.get("IMG_WATCHDOG_TEST") == "1" or "--test" in sys.argv:
        send_telegram(
            token,
            chat_id,
            "✅ IMG Studio watchdog online\nBot đã kết nối. Em sẽ báo lỗi bất thường (fail lặp, job treo, model lệch CPA).",
        )
    ...
    try:
        ...
        # lỗi chính watchdog cũng báo 1 lần
        state = load_state()
        now = time.time()
        key = "watchdog:error"
    ...
    save_state(state)
    log(f"done sent={sent} candidates={len(alerts)}")
    return 0
